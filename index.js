...
http.createServer(async (request, res) => {
  ...
  request.on('end', async () => {
    try {
      ...
      } else if (action === 'market_ltp') {
        const keys = Array.isArray(payload.instrument_key) ? payload.instrument_key.join(',') : payload.instrument_key;
        result = await req({ hostname: 'api.upstox.com', path: `/v2/market-quote/ltp?instrument_key=${encodeURIComponent(keys)}`, method: 'GET', headers: h });
      } else {
