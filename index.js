...
function runSignalEngine(instrument, spot, ohlc, candles, cfg, capital) {
  ...
  // Day range position — use API ohlc or derive from candles
  const effectiveOhlc = ohlc || (candles.length >= 3 ? {
    open:  candles[0].o,
    high:  Math.max(...candles.map(c=>c.h)),
    low:   Math.min(...candles.map(c=>c.l)),
    close: candles[candles.length-1].c
  } : null);
  if (effectiveOhlc?.high && effectiveOhlc?.low && effectiveOhlc?.open) {
    const range = effectiveOhlc.high - effectiveOhlc.low;
    if (range > 0) {
      const pos = (spot - effectiveOhlc.low) / range;
    ...
    if (effectiveOhlc && spot > effectiveOhlc.open)  { reasons.push('Above open — bullish day bias'); score += 8; if (!direction) direction = 'CE'; }
    else if (effectiveOhlc) { reasons.push('Below open — bearish day bias'); score += 8; if (!direction) direction = 'PE'; }
    if (effectiveOhlc && Math.abs(spot-effectiveOhlc.high)/spot < 0.0015) { reasons.push('Near day high — breakout'); score += 12; direction = 'CE'; pattern = 'Breakout'; }
    if (effectiveOhlc && Math.abs(spot-effectiveOhlc.low)/spot < 0.0015)  { reasons.push('Near day low — bounce');    score += 12; direction = 'PE'; pattern = 'Bounce'; }
  }
