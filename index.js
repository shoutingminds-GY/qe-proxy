...
function makeUpstoxRequest(options, body) {
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET') {
    const ipRes = await makeUpstoxRequest({ hostname: 'api.ipify.org', path: '/?format=json', method: 'GET' });
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ status: 'ok', outbound_ip: ipRes.body.ip }));
    return;
  }
  ...
  req.on('end', async () => {
    try {
      ...
      if (action === 'place_order') {
        ...
        result = await makeUpstoxRequest({
